"""Apple mobile device ProductType to friendly product name mapping.

The mapping is generated from the user-provided Apple mobile device type list
based on adamawolf's public device identifier list. If a future or unknown
ProductType is not in the table, ``product_name()`` returns the original
identifier so the UI still shows a useful value.
"""

from __future__ import annotations

DEVICE_TYPE_TO_NAME: dict[str, str] = {
...
def product_name(product_type: str | None) -> str:
    """Return a friendly Apple product name for an internal ProductType.

    Examples:
        iPhone15,4 -> iPhone 15
        iPad16,3 -> iPad Pro 11 inch 5th Gen (WiFi)

    Unknown values are returned unchanged, which keeps the app future-proof
    until the bundled mapping is updated.
    """
    key = (product_type or "").strip()
    if not key:
        return ""
    return DEVICE_TYPE_TO_NAME.get(key, key)
